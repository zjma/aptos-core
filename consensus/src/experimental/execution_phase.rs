...
impl StatelessPipeline for ExecutionPhase {
    ...
    async fn process(&self, req: ExecutionRequest) -> ExecutionResponse {
        ...
        for b in ordered_blocks {
            match self.execution_proxy.compute(b.block(), b.parent_id(), b.maybe_randomness()).await {
                Ok(compute_result) => {
                    result.push(ExecutedBlock::new(b.block().clone(), compute_result, b.maybe_randomness()));
                },
                Err(e) => {
                    return ExecutionResponse {
                        block_id,
